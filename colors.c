...
/*   Updated: 2016/12/28 19:43:16 by nboute           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_convert(float a, float b, float c, char *conv)
{
	...
	{
		color = (t_chsl*)malloc(sizeof(t_chsl));
		((t_chsl*)color)->H = a;
		((t_chsl*)color)->S = b;
		((t_chsl*)color)->L = c;
	...
	{
		color = (t_crgb*)malloc(sizeof(t_crgb));
		((t_crgb*)color)->R = a / (float)255;
		((t_crgb*)color)->G = b / (float)255;
		((t_crgb*)color)->B = c / (float)255;
